...
export default class NetworkManager {
  ...
  async fetchRequest(
    ...
    parameters = {},
    onRetryClicked = null,
    serviceTimeOut = AppInfo.serviceTimeOut,
  ) {
    if (!this.isInternetConnected) {
      console.log('isInternetConnected :' + this.isInternetConnected);
      if (onRetryClicked != null) {
        Utility.sharedInstance.HOC.showOverlay({
          type: 'NO_NETWORK',
    ...
    console.log('http://13.238.112.149:8000/auth/get_allergens/');
    console.log(url);

    console.log(url == 'http://13.238.112.149:8000/auth/get_allergens/');
  ...
  async reachabilityListener(callback) {
    NetInfo.fetch().then((state) => {
      ...
      if (callback) {
        callback(this.isInternetConnected);
      }
    });
